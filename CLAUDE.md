# CLAUDE.md - WebGL Immersive Navigation Framework
...
- **Coverage target:** 100% (global CLAUDE.md contract)
...
| **Trivy** — wired: `aquasecurity/trivy-action@v0.36.0` (`severity: HIGH,CRITICAL`, `exit-code: 1`, `ignore-unfixed: true`) against `webgl-frontend:ci` | `docker-build` job | OS/package CVEs in the `nginx:alpine` runtime image; the build step uses `load: true` so the image is scannable |
